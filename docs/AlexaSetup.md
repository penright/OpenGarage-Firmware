...
This walks you through setting up Opengarage as an Alexa home skill using the lock capability which allows you to query status as well as lock/shut the door. Alexa APIs don't currently support a native skill for doors so a lock is the closest device available. Alexa also doesn't currently support unlocking so this does not allow you to open/unlock the door. You do need to use the term lock and unlock instead of open/shut but the semantics are close.
...
Step 1: Setup an Account here https://alexa-node-red.bm.hardill.me.uk/
=======
Technically this would allow the owner of this site or anyone that has access to it to send a request as if they are Alexa but at least for my house there are easier ways to break in so I don't stress about it. I believe he has the code on github if you want to host your own service. I'm not sure why but neither Ifft or Blynk have similiar capabiliies at the time of this blog.
